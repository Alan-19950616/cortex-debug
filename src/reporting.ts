...
function getUUID(): string {
    if (!uuid) {
        const settingspath = path.join(os.homedir(), '.general-debug');
...
function telemetryEnabled(): boolean {
    ...
    const generaldebug = vscode.workspace.getConfiguration('general-debug');

    return telemetry.get('enableTelemetry', false) && generaldebug.get('enableTelemetry', false);
}
...
function beginSession(id: string, opts: ConfigurationArguments) {
    ...
    analytics.screenview('Debug Session', 'General-Debug', extensionVersion, extensionId);
